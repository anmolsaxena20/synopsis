...
export default function TeamPage() {
  ...
  return (
    <div className="space-y-6 max-w-3xl" style={{ animation: 'fadeIn 0.3s ease-out' }}>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-0.5">They will be added as a Member.</p>
        </div>
        ...
        <div className="px-6 py-5">
          <form onSubmit={handleInvite} className="flex gap-3">
            <input
              ...
              className="flex-1 px-4 py-2.5 text-sm border border-gray-200 rounded-lg outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all duration-150 bg-white text-gray-900 placeholder-gray-400"
            />
            <button
              type="submit"
              disabled={inviting}
              className={`px-5 py-2.5 rounded-lg text-sm font-semibold text-white transition-all duration-150 flex-shrink-0 ${
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">{members.length} {members.length === 1 ? 'member' : 'members'}</span>
        </div>
        ...
        <div className="divide-y divide-gray-50">
          {loading ? (
            ...
            <div className="px-6 py-12 text-center">
              <p className="text-sm text-gray-400">No team members yet. Invite someone above.</p>
            </div>
          ) : (
            members.map(member => (
              <div key={member.id} className="px-6 py-4 flex items-center gap-4 hover:bg-gray-50 transition-all duration-150">
